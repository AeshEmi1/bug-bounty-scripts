...
def parse_args():
    ...
    parser.add_argument('-f', '--domain_file', help='Specify a Target Domain File to get subdomains from crt.sh', required=False)
    parser.add_argument('-o', '--output_file', help='Specify an output file', required=False)
    ...
    parser.add_argument('-s', '--stdout', help='Print output', action='store_true', required=False)
...
def crtsh(domain: str):
    ...
    """
    try:
        response = requests.get(f"https://crt.sh/?q={domain.lstrip().rstrip()}&output=json", timeout=25)
        if response.ok:
